...
namespace VSTODATN.FunctionsExcel
{
    internal class Hust
    {
        ...
        {
            string TenKhongDau = StringFormatter.RemoveAccent(HoVaTen);
            string Email;
            if (MaSoSinhVien.Length == 8)
            {
                string ChuCaiDau = StringFormatter.LayCacChuCaiDau(TenKhongDau);
                Email = StringFormatter.LayTenSV(TenKhongDau) + "." + ChuCaiDau.Substring(0, ChuCaiDau.Length - 1) + MaSoSinhVien.Substring(2, MaSoSinhVien.Length - 2) + "@sis.hust.edu.vn";
            }
            else if (MaSoSinhVien.Length == 0)
            {
                Email = StringFormatter.LayTenSV(TenKhongDau) + "." + StringFormatter.LayHoSV(TenKhongDau) + StringFormatter.LayDemSV(TenKhongDau).Trim() + "@hust.edu.vn";
            }
            else
            {
